 ...
 */
#ifndef UC_DATABASE_MIT_HISTORY_DATABASE_HPP
#define UC_DATABASE_MIT_HISTORY_DATABASE_HPP
...
public:
    ...
    std::shared_ptr<token_mit_info> get(const short_hash& key) const;

    std::shared_ptr<token_mit_info::list> get_history_mits_by_height(const short_hash& key,
        uint32_t start_height = 0, uint32_t end_height = 0,
        uint64_t limit = 0, uint64_t page_number = 0) const;

    std::shared_ptr<token_mit_info::list> get_history_mits_by_time(const short_hash& key,
