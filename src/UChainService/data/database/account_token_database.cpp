...
//#include <UChainService/txs/account/account_token.hpp>
...
account_token_database::account_token_database(const path& lookup_filename,
...
account_token_database::~account_token_database()
{
    close();
}
...
bool account_token_database::create()
{
...
bool account_token_database::start()
{
    return
        lookup_file_.start() &&
        rows_file_.start() &&
...
bool account_token_database::stop()
{
    return
        lookup_file_.stop() &&
        rows_file_.stop();
...
bool account_token_database::close()
{
    return
        lookup_file_.close() &&
        rows_file_.close();
...
void account_token_database::store(const short_hash& key, const token_detail& detail)
{
...
void account_token_database::delete_last_row(const short_hash& key)
{
    rows_multimap_.delete_last_row(key);
}

token_detail::list account_token_database::get(const short_hash& key) const
{
    // Read a row from the data for the account_token list.
...
std::shared_ptr<token_detail> account_token_database::get(const short_hash& key, const std::string& address) const
{
...
std::shared_ptr<business_address_token::list> account_token_database::get_unissued_tokens(const short_hash& key) const
{
    auto result = get(key);
...
void account_token_database::sync()
{
    lookup_manager_.sync();
    rows_manager_.sync();
}

account_token_statinfo account_token_database::statinfo() const
{
    return
    {
        lookup_header_.size(),
